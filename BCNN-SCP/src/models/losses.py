...
def KL_DIV(mu_p, sig_p_inv, sig_p_logdet, mu_q, sig_q):
    ...
    Parameters:
        ...
        sig_p_inv (torch.Tensor): Inverse of the covariance matrix of the prior, shape: (mu_dim, mu_dim)
        sig_p_logdet (torch.Tensor): Log determinant of the covariance matrix of the prior, shape: ()
        mu_q (torch.Tensor): Mean of the posterior, shape: (num_blocks * mu_dim,)
        sig_q (torch.Tensor): Covariance matrix of the posterior, shape: (num_blocks, mu_dim, mu_dim)
    """
    ...
    mu_diff = mu_q.view(num_blocks, mu_dim) - mu_p  # shape: (num_blocks, mu_dim)
    C = torch.einsum("bi,ij,bj->b", mu_diff, sig_p_inv, mu_diff)  # shape: (num_blocks,)

    return -0.5 * torch.sum(A + mu_dim - B - C)
